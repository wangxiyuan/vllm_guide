...
2. 同一请求的尾部 block 先驱逐：尾部 block 是哈希链末端，驱逐它只使自身失效，对前缀共享破坏更小
...
- **同一请求的尾部 block 先驱逐**：释放时按逆序追加（B3, B2, B1, B0），尾部 block 更靠近 LRU 队列头部，先被驱逐。尾部 block 是哈希链末端，驱逐它只使自身失效；而驱逐头部 block 会使整条哈希链断裂，对前缀共享破坏更大。
