...
KV cache 在 GPU 上的存储排布方式。NHD 为逐 block 存储（num_blocks, 2, block_size, num_kv_heads, head_dim），HND 为逐头存储（2, num_blocks, block_size, num_kv_heads, head_dim）。详见 [PagedAttention](../core/paged-attention.html)
...
ZMQ 的通信模式，ROUTER 可异步接收多个 DEALER 的消息并路由回复，用于 API 进程与 EngineCore 间的请求/响应通信。详见 [进程与通信](../core/process-communication.html)

**PUSH/PULL / 推/拉模式**
ZMQ 的单向通信模式，PUSH 端发送消息，PULL 端接收，用于 EngineCore 向 API 进程推送推理输出。详见 [进程与通信](../core/process-communication.html)

**ShmRingBuffer / 共享内存环形缓冲区**
基于 POSIX 共享内存实现的无锁环形缓冲区，用于 EngineCore 与 Worker 间高效传递 SchedulerOutput 和 ModelRunnerOutput。详见 [进程与通信](../core/process-communication.html)

**MessageQueue / 消息队列**
EngineCore 进程内部的线程间通信机制，基于 Python Queue 实现，连接输入线程、核心线程和输出线程。详见 [进程与通信](../core/process-communication.html)
...
将权重矩阵按列切分到多个 GPU，每个 GPU 计算部分输出，gather_output=True 时通过 all-gather 合并。是 TP 的标准线性层切分方式。详见 [并行策略总览](../distributed/parallelism.html)

**RowParallelLinear / 行并行线性层**
将权重矩阵按行切分到多个 GPU，每个 GPU 计算部分结果，reduce_results=True 时通过 all-reduce 或 reduce-scatter 合并。详见 [并行策略总览](../distributed/parallelism.html)
